...
| Consolidate standalone preview features into the Next.js app | Phase 4 in progress | Phase 3 is complete: Next owns the reader HSK visualization/metadata slice, real paste-text/PDF import flow, background extraction progress, compact saved-token definition card, explicit reader loading/error states, sentence-level HSK analysis charts, and book-detail page-level HSK charts. Phase 4 deployment slice is complete: canonical Next runs on `3000`, API integration is configured on `8201`, and the standalone/GitHub Pages compatibility surface remains explicit on `8200`. Remaining Phase 4 work is import-to-reader progression verification, legacy navigation, inventory finalization, and full maintenance-cycle evidence. See `docs/FRONTEND_MIGRATION_PHASE_1.md` through `docs/FRONTEND_MIGRATION_PHASE_4.md`. Affected IDs: `reader.header`, `reader.options-dialog`, `reader.page-card`, `reader.token-inspector`, `reader.sentence-hsk-chart`, `import.form`, `import.progress-card`, `import.recent-books-card`, `import.book-item`, `analysis.sentence-hsk-chart`, `analysis.page-hsk-chart`, `book-detail.page-hsk-chart`. |
