...
| Fix hosted profile 502 during onboarding | #123 | In Progress. Map retryable Supabase hosted-profile failures to logged 503 responses, route an initial settings read failure into onboarding, and allow a learner who has completed the questions to return locally until account storage recovers. Inventory IDs: `onboarding.page`, `onboarding.form`, `onboarding.continue-action`, `profile.hosted-account-card`. |
...
| Add theme store and commerce entitlements | #73 | Parent initiative for the dedicated `/themes` storefront, customer checkout, payment fulfillment, and server-authoritative theme ownership. Catalog discovery, previews, collections, ownership status, and pricing now live separately from personal appearance controls at `/profile/themes`. See `docs/STRIPE_INTEGRATION_PLAN.md`. |
...
| Define language-learning program framework and progression benchmarks | #78 | Shared language-learning program framework covering script handling, proficiency anchors, lexicon sourcing, learner-facing study progression, and non-Romanized language ecosystem benchmarks. Japanese workbook normalization, Afterlife provenance, Memrise Non-WK comparison, and verb-conjugation metadata are documented in `docs/JAPANESE_LEXICON_IMPORT_CONCEPT.md`. |
| Add Japanese conjugation details to Reader and Study | #153 | Todo; Local pending board sync because the authenticated GitHub token lacks `project` scope. Build the learner-facing conjugation grid and practice flow on the engine/API foundation in PR #152; preserve derived-form and learner-state boundaries. See `docs/JAPANESE_LEXICON_IMPORT_CONCEPT.md`. |
...
| Define dynamic pricing and expansion revenue strategy | #140 | Todo. Apply usage- and value-aligned pricing to TextPlex while keeping the core reading loop accessible; define assistance allowances, plan boundaries, expansion paths, experimentation cadence, and economic/privacy guardrails. See `docs/TEXTPLEX_DYNAMIC_PRICING_REPORT.md`. Related to #73 and #97. |
...
| Correct Reader glossing percentage calculation | #128 | Todo. Consolidates records `26b902eb25a142419a135864cb79d037` and `2c0f2e05d2284137a12e721c80afe1d1`; investigate the reported 99% versus approximately 92% Read without glossing value and add regression coverage. |
| Keep Japanese 楽しそう together in Reader | #129 | Todo. Consolidates records `dfe95802fb794fd781ca79871d8a4222` and `f2ee8edc7b5e46c0b9238d3644edc1aa`; preserve the learner-facing unit, reading, and meaning for `楽しそう`. |
| Correct Japanese だった reading in Reader | #130 | Todo. Local record `4fa4b1ca823a43a1bc7d6ef22a23c1b8`; correct the reported `da~tsu` reading to `datta` and add Japanese conjugated-form coverage. |
| Render Japanese romanization in lowercase | #131 | Todo. Local record `9f8f6ed73f694797beed75ae6eea65e5`; preserve intended lowercase romanization output and add focused rendering coverage. |
| Remove stray f from feedback button label | #135 | Done. Local record `6e9ac6fe2428430698b1e069c2ce91e4`; source verified and GitHub issue closed as completed. |
| Correct Japanese 章 romanization in Reader | #136 | Todo. Local record `082a7090a0e440e1b7822af4183560cb`; review contextual reading resolution and preserve alternate readings. |
| Handle repeated section divider markup in Reader | #137 | Todo. Local record `a9db610436d440a7b00bb725c6c33897`; review presentation of repeated `==` markup in sentence-level feedback. |
| Correct Japanese 市 romanization in city context | #138 | Todo. Local record `cbc259b7fb664cdabfe43d71034cb216`; review contextual city-name reading resolution. |
| Correct Japanese January romanization | #139 | Todo. Local record `a56c2da4edb84f25a6ebcb169061868c`; correct the reported `1月` month reading and add regression coverage. |
