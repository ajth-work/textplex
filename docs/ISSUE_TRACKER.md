...
| Add Google Cloud Translation planning, provenance, and language filtering | Local pending | Import now estimates translation usage/cost, prompts for large-text confirmation, exposes a fixed target-language dropdown and library language filters, and adds reader sentence/source translation controls with cached revisit provenance plus pronunciation backfill for lookup hits that fall back to Google. Inventory IDs: `import.translation-confirmation-card`, `reader.sentence-tools`, `reader.source-sentence-card`, `reader.sentence-translation-card`. |
...
| Add Hebrew lexicon sourcing and starter pack | Local pending | Hebrew now has RTL tokenization, transliteration fallback, and reader pronunciation support; the remaining work is to source a canonical Hebrew lexicon pack and encode the common clitic and lemma forms. |
...
| Add Arabic lexicon sourcing and starter pack | Local pending | Arabic now has ACTFL-anchored progression notes, but still needs a Modern Standard Arabic starter pack, transliteration-aware seed list, and first-wave AAPPL topic buckets. |
...
| Add cross-device accounts and authenticated learner profiles | #43 | Add Supabase Auth email/password accounts, Postgres-backed user-scoped learner data, private book/page storage, protected FastAPI routes, and local-profile migration. Auth callback/session restoration now has explicit failure states, LAN development origins are documented, the Next app now gates protected routes behind sign-in, the shell and route headers expose a shared account menu, and the profile surface calls out the active hosted identity plus the user-zero migration path; live account confirmation still depends on Supabase URL Configuration. |
...
| Design adaptive learner-window article generation | #47 | Define a profile-driven article generator that blends known words, recently exposed words, and upcoming study items before import. Related to #23. |
...
| `theme-store.summer-editions` | Complete | Added the three-theme `Summer Editions` collection with Citrus Grove, Sunlit Meadow, and Seaside Garden artwork, semantic palettes, server catalog entries, and a discounted bundle offer. | Each theme is priced at `$1.99`; the three-theme bundle is `$4.99` instead of `$5.97`, saving `$0.98`. |
...
| `theme-store.shop-ui` | In progress | Expand the theme-shop prototype into a scalable Next catalog with server-backed bundle rendering, search, collection filters, Daylight/Night filters, ownership states, empty/error/accessibility/responsive states, and live preview. | Inventory IDs remain mapped and the shop never exposes payment secrets or trusts client prices; detail pages and checkout remain follow-up work. |
