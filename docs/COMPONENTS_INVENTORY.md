...
| `shell.page-guide-trigger` | button | Page guide trigger | `apps/web/components/page-guide.tsx`, `apps/web/app/layout.tsx` | Persistent help control inside the shared build card that reopens the current route's first-visit guide or the general TextPlex guide when a route has no dedicated walkthrough. |
...
| `book-detail.completion-control` | region | Page-by-page completion | Explicitly marks the currently uploaded page frontier finished or reopens it for more pages without archiving the source. |
...
| `reader.session-summary-toggle` | region | Session active summary | Full-width reader session active bar with a swipeable one-row subpill carousel for the current session, glossed counts, coverage, ETA, book-specific metrics, reads, resume point, and progress; completion coverage uses the same readable-word and glossed counts shown to the learner. |
...
| `reader.japanese-conjugation-card` | card | Japanese conjugation details | Reader-only derived conjugation summary for a selected Japanese verb, including its class, rule source, and lexical override note. |
| `reader.japanese-conjugation-grid` | region | Japanese conjugation grid | Explainable grid of generated Japanese verb forms shown inside the Reader token inspector. |
...
| `study.japanese-conjugation-card` | card | Japanese conjugation details | Study presentation of derived Japanese verb forms attached to a practice term without creating learner-state records for those forms. |
| `study.japanese-conjugation-grid` | region | Japanese conjugation grid | Explainable grid of generated Japanese verb forms shown on the Study practice card. |
...
- The app-wide visual density pass keeps shared route heroes, metric rows, repeated card stacks, operational consoles, and the build/feedback footer on one compact spacing rhythm. Shared `surface.route-hero` instances use an editorial content-and-metrics split with the first route action visually prioritized; `landing.hero` gives the Reader preview more visual weight than the supporting Study preview. Library retains the same `library.search-hero` ownership while its Import action aligns with search controls on desktop; Settings and Profile keep their existing IDs while their controls render as bounded rows.
...
| --- | --- | --- |
| `shell.chrome`, `shell.build-footer`, `shell.feedback-footer`, `shell.feedback-button`, `surface.route-hero`, `surface.metrics`, `landing.hero`, `library.search-hero`, `library.import-button`, `activity.pages-progress-chart`, `activity.sentences-progress-chart`, `settings.preferences-card`, `profile.email-change-form`, `admin.nav`, `admin.hero`, `admin-feedback.*`, `admin-theme.*`, `tester.*` | App-wide visual density pass (Local complete) | Tightens duplicated shell and hero spacing, gives shared route heroes an editorial content-and-metrics split, prioritizes the first route action, strengthens the Reader/Study hierarchy on the landing page, gives the two Activity charts the full available width, organizes Settings/Profile controls, aligns the desktop Library import action, and compacts shared admin/tester stacks without changing route ownership or TextPlex's visual identity. |
...
| `reader.japanese-conjugation-card`, `reader.japanese-conjugation-grid`, `study.japanese-conjugation-card`, `study.japanese-conjugation-grid` | [#153](https://github.com/ajth-work/textplex/issues/153) | Reader and Study Japanese conjugation details derived from the PR #152 engine/API foundation; generated forms remain presentation-only and source-rule attribution is visible. |
